	...
	<div class="flex justify-center flex-grow">
	  <div class="bg-white shadow-md rounded-lg w-full max-w-4xl p-4">
		<table class="min-w-full">
		  ...
		  <tbody>
			<tr v-for="vehicle in vehicles" :key="vehicle.vehicle_id" class="border-t">
	  ...
	  <div class="bg-white rounded-lg p-8">
		...
		<form @submit.prevent="addVehicle">
		  ...
		  <button @click="closeModal" type="button" class="ml-2 bg-gray-300 text-black px-4 py-2 rounded hover:bg-gray-400">Cancel</button>
			<button @click="closeAndRefresh" type="button" class="ml-2 bg-orange-500 text-white px-4 py-2 rounded hover:bg-orange-700">Save & Refresh</button>
		</form>
	  </div>
	</div>
  </div>
  ...
  setup() {
	  ...
	  const addVehicle = async () => {
		if (user.value) {
		  ...
		  if (!currentUser || !currentUser.UserID) {
			console.error('Error: Current user object is invalid or missing UserID', currentUser);
			return;
		  }
		  const { data, error } = await supabase.from("Vehicle").insert([{
			user_id: currentUser.UserID,
			vechicle_make: newVehicle.make,
		  ...
		  if (error) {
			...
			console.log('Vehicle added:', data[0]);
			vehicles.value.push(data[0]);
    ...
    const goBack = () => {
	  ...
	  const closeAndRefresh = async () => {
		closeModal();        // Use the existing closeModal method to close the modal
